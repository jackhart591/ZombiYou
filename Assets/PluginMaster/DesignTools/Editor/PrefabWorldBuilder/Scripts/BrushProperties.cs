    ...
    public class BrushProperties : UnityEditor.EditorWindow, ISerializationCallbackReceiver
    {
                            ...
                            using (new GUILayout.HorizontalScope())
                            {
                                ...
                                using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
                                {
                                    brushSelectionSettings.scaleMultiplier = Vector3.one
                                        * UnityEditor.EditorGUILayout.FloatField(brushSelectionSettings.scaleMultiplier.x);
                                    if (check.changed)
                                        brushSelectionState.scaleMultiplier = SelectionFieldState.CHANGED;
                                }
                                GUILayout.FlexibleSpace();
            ...
            UnityEngine.Object objToUndo, string undoMsg)
        {
            ...
            using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
            {
                ...
                if (brushScaleGroupOpen)
                {
                    using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                var value = UnityEditor.EditorGUILayout.FloatField(tempBrush.scaleMultiplier.x);
                ...
                {
                    changed = true;
                    if (PWBCore.staticData.undoBrushProperties) UnityEditor.Undo.RegisterCompleteObjectUndo(objToUndo, undoMsg);
        ...
        private void BrushGroup()
        {
            ...
            using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
            {
                ...
                if (BrushFields(brush, ref _brushPosGroupOpen, ref _brushRotGroupOpen, ref _brushScaleGroupOpen,
        ...
        [SerializeField] private System.Collections.Generic.List<int> _selection
        ...
        private void OpenPrefab(object idx)
            => UnityEditor.AssetDatabase.OpenAsset(PaletteManager.selectedBrush.GetItemAt((int)idx).prefab);
            ...
            using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
            {
                using (var brushCheck = new UnityEditor.EditorGUI.ChangeCheckScope())
                {
                    ...
                    tempBrush.frequencyMode = (MultibrushSettings.FrecuencyMode)
                        UnityEditor.EditorGUILayout.Popup("Frequency Mode:", (int)tempBrush.frequencyMode, FRECUENCY_MODES);
                    ...
                    if (tempBrush.frequencyMode == MultibrushSettings.FrecuencyMode.RANDOM)
                    {
                        using (new GUILayout.HorizontalScope())
                        {
                            if (_selection.Count <= 1)
                            {
                                tempItem.frequency = UnityEditor.EditorGUILayout.FloatField("Frecuency:", tempItem.frequency);
                                GUILayout.Label("in " + tempBrush.totalFrecuency);
                            }
                            else
                            {
                                GUILayout.Box(GetStateGUIContent(_itemSelectionState.frecuency),
                                    UnityEditor.EditorStyles.label);
                                using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
                                {
                                    _itemSelectionSettings.frequency = UnityEditor.EditorGUILayout.FloatField("Frecuency:",
                                        _itemSelectionSettings.frequency);
                                    var totalFrecuency = _itemSelectionSettings.frequency * _selection.Count;
