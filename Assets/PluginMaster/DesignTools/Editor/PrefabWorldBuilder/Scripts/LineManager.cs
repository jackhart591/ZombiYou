...
namespace PluginMaster
{
    ...
    public class LineSegment
    {
        ...
        public System.Collections.Generic.List<Vector3> points = new System.Collections.Generic.List<Vector3>();
    }
    ...
    public static partial class PWBIO
    {
            ...
            if (askIfWantToSave)
            {
                void Save()
                {
                    if (UnityEditor.SceneView.lastActiveSceneView != null)
                        LineStrokePreview(UnityEditor.SceneView.lastActiveSceneView.camera, _lineData, false, true);
                    CreateLine();
                }
            ...
            out bool removedFromSelection, out bool wasEdited, out Vector3 delta)
        {
            ...
            if (showHandles && lineData.showHandles && lineData.selectedPointIdx >= 0)
            {
                ...
                lineData.SetPoint(lineData.selectedPointIdx,
                    UnityEditor.Handles.PositionHandle(lineData.selectedPoint, Quaternion.identity), false);
        ...
        private static void LineToolEditMode(UnityEditor.SceneView sceneView)
        {
            ...
            if (someLinesWereEdited && linesEdited.Length > 1)
            {
                ...
                foreach (var lineData in linesEdited)
                {
                    ...
                    LineStrokePreview(sceneView.camera, lineData, true, true);
                }
            ...
            {
                var forceStrokeUpdate = _updateStroke;
                if (_updateStroke)
                {
                ...
                LineStrokePreview(sceneView.camera, _selectedPersistentLineData, true, forceStrokeUpdate);
        ...
        private static void PreviewPersistenLine(LineData lineData)
        {
            ...
            for (int objIdx = 0; objIdx < objPos.Length; ++objIdx)
            {
                ...
                var pivotToCenter = obj.transform.InverseTransformPoint(bounds.center);
                ...
                var itemPosition = objPos[objIdx] + segmentDir / 2;
        ...
        private static void LineStateStraightLine(bool in2DMode)
        {
            ...
            UnityEditor.Handles.DrawAAPolyLine(8, new Vector3[] { _lineData.GetPoint(0), _lineData.GetPoint(3) });
            UnityEditor.Handles.color = new Color(1f, 1f, 1f, 0.7f);
            UnityEditor.Handles.DrawAAPolyLine(4, new Vector3[] { _lineData.GetPoint(0), _lineData.GetPoint(3) });
            DrawDotHandleCap(_lineData.GetPoint(0));
        ...
        private static void LineInput(bool persistent)
        {
                ...
                && Event.current.control && Event.current.shift)
            {
                ...
                || (Event.current.keyCode == KeyCode.Y && Event.current.control && Event.current.shift)))
            {
                lineData.ToggleSegmentType();
                _updateStroke = true;
            }
            else if (Event.current.type == EventType.KeyDown && (Event.current.keyCode == KeyCode.End
                || (Event.current.keyCode == KeyCode.O && Event.current.control && Event.current.shift)))
            {
                ...
                && Event.current.shift && Event.current.control)
            {
                ...
                LineManager.settings.gapSize += lineData.lenght * deltaSign * 0.001f;
                ToolProperties.RepainWindow();
                Event.current.Use();
            }
            if (!persistent) return;
            if (Event.current.type == EventType.KeyDown && Event.current.control && Event.current.shift
                && Event.current.keyCode == KeyCode.T && lineData != null)
            {
                var parent = lineData.GetParent();
                if (parent != null) UnityEditor.Selection.activeGameObject = parent;
            }
            else if (Event.current.type == EventType.KeyDown && Event.current.alt && Event.current.keyCode == KeyCode.Delete)
            {
                ToolProperties.RegisterUndo("Delete Line");
                LineManager.instance.DeletePersistentItem(lineData.id);
            }
        }
        private static void LineStateBezier(UnityEditor.SceneView sceneView)
        {
            ...
            LineStrokePreview(sceneView.camera, _lineData, false, forceStrokeUpdate);
            DrawLine(_lineData);
            DrawSelectionRectangle();
            LineInput(false);
        ...
        private static void LineStrokePreview(Camera camera, LineData lineData, bool persistent, bool forceUpdate)
        {
            var settings = lineData.settings;
