...
namespace PluginMaster
{
    ...
    public class MultibrushSettings : BrushSettings, ISerializationCallbackReceiver
    {
        ...
        [SerializeField] private System.Collections.Generic.List<MultibrushItemSettings> _items
        ...
        private Texture2D _thumbnail = null;
        private float _totalFrequency = -1;
    ...
    {
        [SerializeField] private System.Collections.Generic.List<MultibrushSettings> _brushes
    ...
    {
        [SerializeField]
        private System.Collections.Generic.List<PaletteData> _paletteData = new System.Collections.Generic.List<PaletteData>()
        { new PaletteData("Palette", System.DateTime.Now.ToBinary()) };
        public static PaletteData[] paletteData => instance._paletteData.ToArray();
        ...
        {
            instance._paletteData.Clear();
            instance._paletteData.Add(new PaletteData("Palette", System.DateTime.Now.ToBinary()));
        ...
        {
            instance._paletteData.Add(palette);
            PWBCore.staticData.Save();
        }

        public static void RemovePaletteAt(int paletteIdx)
        {
            instance._paletteData.RemoveAt(paletteIdx);
            PWBCore.staticData.Save();
        }
        ...
        {
            if (from == to) return;
            instance._paletteData.Insert(to, instance._paletteData[from]);
            var removeIdx = from;
            if (from > to) ++removeIdx;
            instance._paletteData.RemoveAt(removeIdx);
        }
        ...
        public static string[] paletteNames => instance._paletteData.Select(p => p.name).ToArray();
        ...
        public static int selectedPaletteIdx
        {
            get
            {
                instance._selectedPaletteIdx = Mathf.Clamp(instance._selectedPaletteIdx, 0,
                    Mathf.Max(instance._paletteData.Count - 1, 0));
        ...
        public static PaletteData selectedPalette => instance._paletteData[instance._selectedPaletteIdx];

        public static int paletteCount => instance._paletteData.Count;

        public static MultibrushSettings GetBrushById(long id)
        {
            foreach (var palette in instance._paletteData)
