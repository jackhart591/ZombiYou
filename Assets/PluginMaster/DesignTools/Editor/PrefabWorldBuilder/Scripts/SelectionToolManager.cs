...
namespace PluginMaster
{
    ...
    public static partial class PWBIO
    {
        ...
        private static void SelectionDuringSceneGUI(UnityEditor.SceneView sceneView)
        {
            ...
            if (UnityEditor.Tools.current != UnityEditor.Tool.None) UnityEditor.Tools.current = UnityEditor.Tool.None;
        ...
        private static void SelectionInput(System.Collections.Generic.List<Vector3> points, bool in2DMode)
        {
            ...
            else if ((keyCode == KeyCode.J || keyCode == KeyCode.UpArrow) && Event.current.type == EventType.KeyDown
                && Event.current.control && !Event.current.alt && Event.current.shift)
                RotateSelection90Deg(Vector3.right, points);
            else if ((keyCode == KeyCode.U || keyCode == KeyCode.DownArrow) && Event.current.type == EventType.KeyDown
                && Event.current.control && !Event.current.alt && Event.current.shift)
                RotateSelection90Deg(Vector3.left, points);
            else if ((keyCode == KeyCode.L || keyCode == KeyCode.RightArrow) && Event.current.type == EventType.KeyDown
                && Event.current.control && Event.current.alt && !Event.current.shift)
                RotateSelection90Deg(Vector3.up, points);
            else if ((keyCode == KeyCode.K || keyCode == KeyCode.LeftArrow) && Event.current.type == EventType.KeyDown
               && Event.current.control && Event.current.alt && !Event.current.shift)
                RotateSelection90Deg(Vector3.down, points);
            else if ((keyCode == KeyCode.U || keyCode == KeyCode.UpArrow) && Event.current.type == EventType.KeyDown
              && Event.current.control && Event.current.alt && !Event.current.shift)
                RotateSelection90Deg(Vector3.forward, points);
            else if ((keyCode == KeyCode.J || keyCode == KeyCode.DownArrow) && Event.current.type == EventType.KeyDown
              && Event.current.control && Event.current.alt && !Event.current.shift)
        ...
        private static void OnBrushChanged() => ApplySelectionFilters();
            ...
            System.Collections.Generic.List<Vector3> points, UnityEditor.SceneView sceneView)
        {
            ...
            points[_selectedBoxPointIdx] = SnapAndUpdateGridOrigin(points[_selectedBoxPointIdx],
                SnapManager.settings.snappingEnabled, true, true, true);
