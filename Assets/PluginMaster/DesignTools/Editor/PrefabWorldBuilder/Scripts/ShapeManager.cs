    ...
    public static partial class PWBIO
    {
            ...
            if (askIfWantToSave)
            {
                void Save()
                {
                    if (UnityEditor.SceneView.lastActiveSceneView != null)
                        ShapeStrokePreview(UnityEditor.SceneView.lastActiveSceneView.camera, ShapeData.nextHexId, true);
                    CreateShape();
                }
        ...
        private static void ShapeToolEditMode(UnityEditor.SceneView sceneView)
        {
            ...
            if (_editingPersistentShape && _selectedPersistentShapeData != null)
            {
                ...
                ShapeStrokePreview(sceneView.camera, _selectedPersistentShapeData.hexId, forceStrokeUpdate);
            }
            if (Event.current.type == EventType.KeyDown && Event.current.keyCode == KeyCode.Return)
            {
                ...
                && Event.current.keyCode == KeyCode.T && _selectedPersistentShapeData != null)
            {
            ...
            out bool wasEdited, bool showHandles, out Vector3 delta)
        {
            ...
            if (showHandles && shapeData.selectedPointIdx >= 0)
            {
                ...
                var snappedPoint = UnityEditor.Handles.PositionHandle(prevPosition, shapeData.planeRotation);
                ...
                if (shapeData.selectedPointIdx == 0)
                {
                    ...
                    var rotation = UnityEditor.Handles.RotationHandle(prevRotation, shapeData.center);
        ...
        private static void ShapeStateEdit(Camera camera)
        {
            ...
            var forceUpdate = _updateStroke;
            if (_updateStroke)
            {
                BrushstrokeManager.UpdateShapeBrushstroke();
                _updateStroke = false;
            }
            ShapeStrokePreview(camera, ShapeData.nextHexId, forceUpdate);
            ...
            if (_shapeData.selectedPointIdx >= 0)
            {
                ...
                var snappedPoint = UnityEditor.Handles.PositionHandle(prevPosition, _shapeData.planeRotation);
                snappedPoint = SnapAndUpdateGridOrigin(snappedPoint, SnapManager.settings.snappingEnabled,
                ...
                if (_shapeData.selectedPointIdx == 0)
                {
                    ...
                    var rotation = UnityEditor.Handles.RotationHandle(prevRotation, _shapeData.center);
                    if (prevRotation != rotation)
                    {
        ...
        private static void ShapeStrokePreview(Camera camera, string hexId, bool forceUpdate)
        {
