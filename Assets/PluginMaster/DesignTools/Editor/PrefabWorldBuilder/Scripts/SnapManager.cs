...
namespace PluginMaster
{
    ...
    public static partial class PWBIO
    {
            ...
            else if (SnapManager.settings.showScaleHandle)
            {
                if (SnapManager.settings.radialGridEnabled)
                {
                    ...
                    var step = UnityEditor.Handles.ScaleHandle(step0, originOffset,
                        rotation, UnityEditor.HandleUtility.GetHandleSize(originOffset));
                    if (step0 != step)
                    {
                ...
                else
                {
                    ...
                    originOffset, rotation, UnityEditor.HandleUtility.GetHandleSize(originOffset));
                }
            }
        ...
        private static void DrawGrid(AxesUtils.Axis axis, Vector3 focusPoint, int maxCells, Vector3 snapSize)
        {
            ...
            var focusOffset = focusPoint - SnapManager.settings.origin;
            ...
            focusPoint = SnapManager.settings.origin + Vector3.Scale(focusOffsetInt, snapSize);
        ...
        {
            if (Event.current.type == EventType.KeyDown && Event.current.control && Event.current.keyCode == KeyCode.G)
            {
                if (!_gridShorcutEnabled) _gridShorcutEnabled = true;
                else
                {
            ...
            if (_gridShorcutEnabled)
            {
                if (Event.current.type == EventType.KeyDown && Event.current.control)
                {
                    ...
                    else if (Event.current.keyCode == KeyCode.W && !Event.current.alt && !Event.current.shift
                    ...
                    else if (Event.current.keyCode == KeyCode.E && !Event.current.alt && !Event.current.shift
                        && UnityEditor.Selection.activeTransform != null)
                    {
                    ...
                    else if (Event.current.keyCode == KeyCode.R && !Event.current.alt && !Event.current.shift
                        && UnityEditor.Selection.activeTransform != null)
                    {
