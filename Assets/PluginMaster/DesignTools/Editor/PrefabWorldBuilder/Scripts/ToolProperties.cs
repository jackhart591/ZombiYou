...
namespace PluginMaster
{
    public class ToolProperties : UnityEditor.EditorWindow
    {
        ...
        {
            if (_instance != null) _instance.SelectProfile(profileData);
        }
            ...
            IPaintToolSettings paintSettings)
        {
            using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
            {
                ...
                using (new GUILayout.HorizontalScope())
                {
                    ...
                    using (new UnityEditor.EditorGUI.DisabledGroupScope(!paintOnSurfaceSettings.paintOnMeshesWithoutCollider))
        ...
        private void LineBaseGUI<SETTINGS>(SETTINGS lineSettings) where SETTINGS : LineSettings
        {
            ...
            using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
            {
                ...
                using (var check = new UnityEditor.EditorGUI.ChangeCheckScope())
                {
                    _lineProjDirIdx = UnityEditor.EditorGUILayout.Popup("Pojection Direction:", _lineProjDirIdx, dirNames);
                    if (check.changed)
                    {
                ...
                if (mirrorSettings.action == MirrorSettings.MirrorAction.CREATE)
                {
                    using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
                    {
                        mirrorSettings.sameParentAsSource
                            = UnityEditor.EditorGUILayout.ToggleLeft("Same parent as source", mirrorSettings.sameParentAsSource);
        ...
        private void ReplacerGroup()
        {
            ...
            using (new GUILayout.VerticalScope(UnityEditor.EditorStyles.helpBox))
            {
                ...
                if (settings.overwriteBrushProperties) BrushProperties.BrushFields(settings.brushProperties,
                    ref _replacerOverwriteGroupState.brushPosGroupOpen, ref _replacerOverwriteGroupState.brushRotGroupOpen,
                    ref _replacerOverwriteGroupState.brushScaleGroupOpen, this, UNDO_MSG);
            }
